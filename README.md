...
Спочатку я визначила типи даних і змінила колонку "CustomerID" з текстового типу на ціле число. Також колонку з номером телефону змінила з числового типу на текстовий.

Далі у колонках "FirstName", "LastName" і "City" прибрала зайві пробіли на початку та в кінці значень, а також зробила перші літери слів великими.

Також видалила дублікати в колонках "CustomerID" та "Email".

2. Order_Details

Для початку змінила тип даних у колонках "OrderDetailsID", "OrderID" та "Quantity" з текстового на ціле число. Далі у колонках "ProductID", "BasePrice", "Discount", "ShippingCost" і "Tax" замінила крапку на кому, щоб змінити тип даних із текстового на десятковий.

У колонках "BasePrice" і "ProductID" видалила значення "null", оскільки такі дані можуть призвести до некоректних результатів.

Колонку "OrderStatus" очистила від зайвих пробілів на початку та в кінці, а також зробила перші літери слів великими.

Після цього перевірила наявність дублікатів у колонці "OrderDetailsID" та видалила їх, оскільки ці значення мають бути унікальними.

3. Order_Metadata

У даній таблиці спочатку перетворила колонку "Meta" на 9 окремих колонок ("carrier", "service", "tracking", "city", "type", "os", "source", "campaign", "notes") для зручності роботи з даними.

Далі колонки "carrier", "service" і "city" очистила від зайвих пробілів та зробила перші літери слів великими.

У колонці "OrderID" видалила дублікати.

Також у колонці "notes" замінила значення "null" на «не вказано».

Для колонок "tracking", "type", "os", "source", "campaign" і "notes" встановила текстовий тип даних.

4. Orders

У колонці "CustomerID" видалила порожні комірки, оскільки ця інформація є обов’язковою. Також у цій колонці замінила крапку на кому в десяткових числах та змінила тип даних із текстового на десятковий.
У колонці "OrderAmount" також замінила крапку на кому в десяткових числах і змінила тип даних на десятковий.

5. Payments

Для початку видалила всі порожні значення в колонці "OrderID", оскільки платіж без замовлення не має сенсу.

Також видалила пропуски в колонці "ShippingDate".

У колонках "OrderID", "PaymentAmount" та "RefundAmount" замінила крапку на кому в числах, щоб коректно змінити тип даних і уникнути помилок. Для цих колонок встановила десятковий тип даних.

Після цього колонку "OrderID" перетворила з десяткового типу на ціле число.

У колонках "PaymentMethod" та "PaymentStatus" прибрала зайві пробіли на початку й у кінці значень, а також зробила перші літери слів великими.

Також видалила дублікати в колонці "OrderID".

6. Products

У колонці "ProductPrice" видалила порожні значення, оскільки ціна є важливим атрибутом товару.

Колонки "ProductName" і "ProductCategory" очистила від зайвих пробілів, а перші літери кожного слова зробила великими
